// (updated) js/saveSlots.js
// Added saving/loading for Voice Pitch and Bladder Control so they persist to presets.
...
function saveSettings(x) {
	...
	// Persist male-specific prefs
	localStorage.setItem("Slot" + x + "_Morning_Wood", localStorage.getItem("Slot0_Morning_Wood"));
	localStorage.setItem("Slot" + x + "_Erection_Firmness", localStorage.getItem("Slot0_Erection_Firmness"));
	localStorage.setItem("Slot" + x + "_Sperm_Count", localStorage.getItem("Slot0_Sperm_Count"));
	localStorage.setItem("Slot" + x + "_Penile_Length", localStorage.getItem("Slot0_Penile_Length"));
	localStorage.setItem("Slot" + x + "_Testicle_Size", localStorage.getItem("Slot0_Testicle_Size"));
	// Labels
	var i;
	for (i = 1; i < 11; i++) {
	...
	// Persist new items: Voice Pitch and Bladder Control
	// Voice pitch is stored in Slot0_Voice_Pitch by the radio onclick handlers.
	localStorage.setItem("Slot" + x + "_Voice_Pitch", localStorage.getItem("Slot0_Voice_Pitch"));
	// Bladder slider value (mySliderBladder)
	if (typeof mySliderBladder !== "undefined" && mySliderBladder !== null) {
		localStorage.setItem("Slot" + x + "_Bladder_Control", mySliderBladder.getValue());
	}
...
function loadSettings(x) {
	// Restore male-specific prefs
	var morningWood = localStorage.getItem("Slot" + x + "_Morning_Wood");
	if (morningWood === "Yes") {
		var cb = document.getElementById("morningWoodCheckbox");
		if (cb) { cb.checked = true; }
		localStorage.setItem("Slot0_Morning_Wood", "Yes");
	} else {
		var cb = document.getElementById("morningWoodCheckbox");
		if (cb) { cb.checked = false; }
		localStorage.setItem("Slot0_Morning_Wood", "No");
	}

	var val;
	val = localStorage.getItem("Slot" + x + "_Erection_Firmness");
	if (val != null && typeof mySliderErection !== "undefined" && mySliderErection !== null) {
		mySliderErection.setValue(val);
		localStorage.setItem("Slot0_Erection_Firmness", val);
	}
	val = localStorage.getItem("Slot" + x + "_Sperm_Count");
	if (val != null && typeof mySliderSperm !== "undefined" && mySliderSperm !== null) {
		mySliderSperm.setValue(val);
		localStorage.setItem("Slot0_Sperm_Count", val);
	}
	val = localStorage.getItem("Slot" + x + "_Penile_Length");
	if (val != null && typeof mySliderPenile !== "undefined" && mySliderPenile !== null) {
		mySliderPenile.setValue(val);
		localStorage.setItem("Slot0_Penile_Length", val);
	}
	val = localStorage.getItem("Slot" + x + "_Testicle_Size");
	if (val != null && typeof mySliderTesticle !== "undefined" && mySliderTesticle !== null) {
		mySliderTesticle.setValue(val);
		localStorage.setItem("Slot0_Testicle_Size", val);
	}
	...
	if (lock.style.display === "block") {
		...
		if (Species == null) {
			...
			// Load voice pitch (new radio input)
			var voicePitch = localStorage.getItem("Slot" + x + "_Voice_Pitch");
			if (voicePitch === "High pitch") {
				var rbHigh = document.getElementById("radiobuttonVoiceHigh");
				if (rbHigh) { rbHigh.checked = true; }
				localStorage.setItem("Slot0_Voice_Pitch", "High pitch");
			}
			else if (voicePitch === "Low pitch") {
				var rbLow = document.getElementById("radiobuttonVoiceLow");
				if (rbLow) { rbLow.checked = true; }
				localStorage.setItem("Slot0_Voice_Pitch", "Low pitch");
			}
			...
			// Load bladder control (new slider)
			var bladderVal = localStorage.getItem("Slot" + x + "_Bladder_Control");
			if (bladderVal != null && typeof mySliderBladder !== "undefined" && mySliderBladder !== null) {
				mySliderBladder.setValue(bladderVal);
				localStorage.setItem("Slot0_Bladder_Control", bladderVal);
			}
